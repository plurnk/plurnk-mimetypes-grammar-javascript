#!/usr/bin/env node
// Reproducible WASM build for tree-sitter-javascript.
...
const work = await mkdtemp(path.join(tmpdir(), "grammar-javascript-build-"));
await run("git", ["clone", "--no-checkout", "https://github.com/tree-sitter/tree-sitter-javascript.git", "src"], { cwd: work });
...
// Locate produced wasm and copy as javascript.wasm.
const fs = await import("node:fs/promises");
const built = (await fs.readdir(buildCwd)).find((f) => f.endsWith(".wasm"));
if (!built) throw new Error("no .wasm produced");
await copyFile(path.join(buildCwd, built), path.join(repoRoot, "javascript.wasm"));
const bytes = (await readFile(path.join(repoRoot, "javascript.wasm"))).length;
console.log(`javascript.wasm: ${bytes} bytes (built from ${pin})`);
