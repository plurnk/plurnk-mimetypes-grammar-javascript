#!/usr/bin/env node
// CI byte-identical reproducibility check for javascript.wasm.
...
const committed = await readFile(path.join(repoRoot, "javascript.wasm"));
const committedHash = createHash("sha256").update(committed).digest("hex");
console.log(`committed javascript.wasm sha256: ${committedHash}`);

const work = await mkdtemp(path.join(tmpdir(), "grammar-javascript-verify-"));
...
console.log(`rebuilt javascript.wasm sha256: ${rebuiltHash}`);
